    ...
    <div class="change-password">
        <div class="change-password__left">
            ...
            <div class="change-password__bottom">
                <h1 class="change-password__title">Изменение пароля</h1>
                <div class="change-password__form">
                    <div class="change-password__wrapper input__wrapper">
                        <transition name="fade">
                            <div v-if="form.old.error" class="input__error">
                        ...
                        <input
                            ...
                            :class="{'t-input__error' : form.old.error}"
                            @input="form.old.error = false; error = ''"
                        >
                    </div>
                    <!-- ////////////////////-->
                    <div class="change-password__wrapper input__wrapper">
                        <transition name="fade">
                            <div v-if="form.new.error" class="input__error">
                        ...
                        <input
                            ...
                            :class="{'t-input__error' : form.new.error}"
                            @input="form.new.error = false; error = ''"
                        >
                    </div>
                    <!-- /////////////////////// -->
                    <div class="change-password__wrapper input__wrapper">
                        <transition name="fade">
                            <div v-if="form.repeat.error" class="input__error">
                                Подтверждение
                            </div>
                        </transition>
                        <input
                            v-model="form.repeat.value"
                            type="password"
                            class="input"
                            placeholder="Старый пароль"
                            :class="{'t-input__error' : form.repeat.error}"
                            @input="form.repeat.error = false; error = ''"
                        >
                    </div>
                    <div class="change-password__hints">
                        <p class="change-password__text change-password__subtitle">Пароль должен содержать:</p>
                        <ul>
                            <li :class="{'change-password__hint_ready': hints[0]}" class="change-password__hint change-password__text">Более 8 символов</li>
                            <li :class="{'change-password__hint_ready': hints[1]}" class="change-password__hint change-password__text">1 цифру</li>
                            <li :class="{'change-password__hint_ready': hints[2]}" class="change-password__hint change-password__text">1 заглавную букву</li>
                            <li class="change-password__hint change-password__text">1 специальный символ (! & и т.д.)</li>
                        </ul>
                    </div>
                    <!-- <div :class="{'change-password__error_hidden': error == ''}" class="change-password__error">{{error}}</div> -->
                    <div @click="sendForm" class="change-password__enter">Отправить</div>
                </div>
            </div>
        </div>
        <div class="change-password__right">
            <img src="/src/assets/images/micro.png" alt="" class="change-password__img">
        </div>
    </div>
</template>
...
export default {
    ...
    computed: {
        hints() {
            ...
            if(this.form.new.value.length > 8) {
                arr[0] = true
            }

            if(/[0-9]/.test(this.form.new.value)) {
                arr[1] = true
            }

            if(this.form.new.value.toLowerCase() != this.form.new.value) {
                arr[2] = true
            }
    ...
    methods: {
        ...
        sendForm() {
            ...
            if(this.validForm()) {
                console.log('valid');
                let action = 'user/profile/password_ajax'
                let method = 'post'
                let json = {
                    action: 'save',
                    data: {
                        password: this.new,
                        password_retype: this.repeat
                    },
...
@import '~@styles/pages/change-password.sass'
</style>